"""Phase 2 — GNSS v2 channel (gnss_v2): a properly-engineered replacement for the
broken single-scalar gnss_rate_change (gnss_coupling, kept for the audit trail).

Fixes every v1 flaw:
  (1) v1 took hypot(E,N) BEFORE detrending -> the secular plate motion dominated the
      "rate". v2 models EAST and NORTH SEPARATELY (direction is essential to remove
      the secular trend); the norm is taken only AFTER detrending.
  (2) v1 ignored equipment/earthquake steps -> v2 subtracts known step offsets
      (NGL steps.txt) inside the trajectory model.
  (3) v1 had no seasonal terms -> v2 fits annual + semiannual.
  (4) v1 used the single nearest station -> v2 inverse-distance^2 weights stations
      within 60 km (>= 2, else 0).
  (5) v1 cut causality at < t0 -> v2 fits the trajectory on epochs < t0-365d and
      derives ALL features from the residuals of [t0-365d, t0) against the
      EXTRAPOLATED model. Every feature for window t0 therefore uses only data < t0,
      making recompute_truncated bit-for-bit trivial (the leakage tripwire).
...
class GnssV2Source(FeatureSource):
    name = "gnss_v2"
    ...
    data_spec = {
        ...
        "fetch": "sources/fetch_data.py gnss_v2  (Nevada Geodetic Lab, free, no key)",
