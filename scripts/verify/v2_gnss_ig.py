"""V2(c) + V2(d) — GNSS forensic audit: bootstrap CI on the aug-vs-base IG, and
permutation importance of the gnss_v2 columns.
...
(d) Permutation importance of each gnss_v2 column within the trained aug model: if
    the gain loads on gnss_strain_fallback (availability proxy) rather than the
    residual-rate / strain features -> artifact -> VOID.

Writes results/verify/v2_gnss_ig.json.
"""
import json
import numpy as np
import pandas as pd
...
def main():
    ...
    json.dump(out, open(VER / "v2_gnss_ig.json", "w"), indent=2)
