...
Reads the event-level stochastic catalogues written by scripts/csep_prep.py, builds
...
Run in the CSEP environment (see requirements-csep.txt):
    MPLBACKEND=Agg python scripts/csep_run.py
"""
...
def _write_md(res, path):
    ...
    L += ["", "## Interpretation", "",
          f"**The N-test and M-test are the robust, citable results and agree with the in-house "
          f"implementation.** The cascade and sv_etas forecasts {ndir} the observed count "
          f"({cfc:.0f} and {sfc:.0f} vs {nobs} observed M>=3.0 events) and {nver} the N-test, but "
          f"remain {mver} (M-test); `modern_etas` (Mizrahi first-gen, no secondary triggering) "
          f"under-counts and also fails the N-test.",
          "",
          ...
          "pseudo-likelihood is far more extreme than any realisation. This is a property of the "
          "catalogue approximation, not evidence against the spatial forecast; a native-clustered-"
          "catalogue S-test is future work (it would require modifying the hash-protected cascade "
          "reproduction path).",
          "",
          f"**Bottom line:** genuine pyCSEP confirms the in-house result. Over the test period the "
          f"cascade and sv_etas forecasts are {mver} but {ndir} the M>=3.0 count and {nver} the "
          f"N-test; the independent Mizrahi first-gen (`modern_etas`) under-counts and fails both. "
          f"The paper cites these pyCSEP numbers; the in-house implementation is retained as a "
          f"cross-check (N/M verdicts agree for all three models)."]
    path.write_text("\n".join(L))
