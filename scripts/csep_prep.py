"""Generate pyCSEP inputs (event-level catalogs) from the in-house model rates,
replicating csep_eval's exact rng sequence so the pyCSEP N/M numbers cross-check the
in-house ones.
...
The driver scripts/csep_run.py loads these into pyCSEP CSEPCatalog/CatalogForecast
objects and runs the number/magnitude/spatial/PL tests.
Run:  PYTHONPATH=src .venv/bin/python scripts/csep_prep.py
"""
