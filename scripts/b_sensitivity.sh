...
# the honest operational b (1.15) and its committed artifacts at the end.
set -u
cd "$(dirname "$0")/.." || exit 1
export PYTHONPATH=src COPYFILE_DISABLE=1
PY=.venv/bin/python
REPORT=results/etas_fit_report.json
HONEST=1.15
SCRATCH="$(mktemp -d)"
setb(){ $PY -c "import json;p='$REPORT';r=json.load(open(p));r['operational_b_for_cascade']=float('$1');json.dump(r,open(p,'w'),indent=2)"; }
corerun(){ $PY -m marmara.grid_hybrid >/dev/null 2>&1; $PY -m marmara.etas_rates results/etas_sv_params.pkl sv_etas >/dev/null 2>&1; $PY -m marmara.train >/dev/null 2>&1; $PY -m marmara.bootstrap >/dev/null 2>&1; }

echo "=== b-sensitivity sweep $(date) ==="
# current committed state is the honest b=1.15
...
echo ">>> restoring honest b=$HONEST @ $(date +%H:%M:%S)"; setb "$HONEST"; corerun
$PY -m marmara.csep_eval >/dev/null 2>&1; $PY scripts/csep_prep.py >/dev/null 2>&1
...
json.dump(out, open("results/b_sensitivity.json","w"), indent=2)
L = ["# Operational-b sensitivity: the ML hybrid is unstable, the physics is not","",
...
for t in ("y30","y35"):
    ...
    for p in PAIRS_PP:
        L.append("| "+p.replace("_vs_"," vs ")+" | "+" | ".join(verdict(b,p,t) for b in bs)+" |")
    L.append("")
open("results/b_sensitivity.md","w").write("\n".join(L))
